...
probEat <- function(alpha, beta, gamma, sp_mass, j, i) {
  log_ratio_squared <- (log(sp_mass$f[j] / sp_mass$f[i]))^2
  logit_prob <- alpha + beta * log(sp_mass$f[j] / sp_mass$f[i]) + gamma * log_ratio_squared
  probability <- inv_logit(logit_prob)
  round(probability, 2)
}

probEat(alpha, beta, gamma, sp_mass, 12, 11)
```
...
feeding_matrix <- matrix(0, nrow = num_species, ncol = num_species)
rownames(feeding_matrix) <- row_labs_sp
colnames(feeding_matrix) <- row_labs_sp

for (i in 1:num_species) {
  for (j in 1:num_species) {
    feeding_matrix[i, j] <- probEat(alpha, beta, gamma, sp_mass, i, j)
  }
}
...
feeding_matrix <- feeding_matrix * carnivores
feeding_matrix <- t(t(feeding_matrix) * carnivores)

print(list("Row sums" = rowSums(feeding_matrix), "Column sums" = colSums(feeding_matrix)))
```
...
```{r t1-time-webs}
present_in_period_2 <- sp_occ[, 2] > 0

feeding_matrix_period_2 <- feeding_matrix[present_in_period_2, present_in_period_2]

dim(feeding_matrix_period_2)

# feeding_matrix_period_2 <- empty(feeding_matrix_period_2)

sum(feeding_matrix_period_2 > 0)

dim(feeding_matrix_period_2)[1]

feeding_matrix_period_2
```
...
for (time in 1:ncol(sp_occ)) {
  present_in_period <- sp_occ[, time] > 0
  
  feeding_matrix_period <- feeding_matrix[present_in_period, present_in_period]
  
  feeding_matrix_period <- empty(feeding_matrix_period)
  
  network_list[[time]] <- feeding_matrix_period
}

network_list[[5]][1:5, 1:5]
```
...
```{r graphs}
igraph_networks <- vector("list", length(network_list))

for (i in 1:length(network_list)) {
  igraph_networks[[i]] <- graph_from_adjacency_matrix(network_list[[i]], mode = "directed", diag = FALSE, weighted = TRUE)
}
```
...
species_groups <- ifelse(carnivores == 1, "Carnivore", "Herbivore")
## plot the network
plot(igraph_networks[[1]], 
     vertex.color = ifelse(species_groups == "Carnivore", "red", "green"), 
     vertex.size = 10, 
     edge.arrow.size = 0.5,
     main = "Network 1: Time Period 1")
```

```{r plot-g2}
plot(igraph_networks[[2]], 
     vertex.color = ifelse(species_groups == "Carnivore", "red", "green"), 
     vertex.size = 10, 
     edge.arrow.size = 0.5,
     main = "Network 2: Time Period 2")
```

```{r plot-g3}
plot(igraph_networks[[3]], 
     vertex.color = ifelse(species_groups == "Carnivore", "red", "green"), 
     vertex.size = 10, 
     edge.arrow.size = 0.5,
     main = "Network 3: Time Period 3")
```
